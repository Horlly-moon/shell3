...
/**
 * isChain - it test if current character in buffer is a chain delimeter
...
/**
 * checkChain - it checks if we should continue chaining based on last status
 * @info: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
...
/**
 * replaceAlias - it replaces an aliases in the tokenized string
 * @info: the parameter struct
 *
...
/**
 * replaceVars - it replaces variables in the tokenized string
 * @info: the parameter struct
 *
 * Return: 1 if replaced, 0 otherwise
 */
...
/**
 * replaceString - it replaces a string
